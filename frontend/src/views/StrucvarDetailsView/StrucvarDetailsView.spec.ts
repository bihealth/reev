...
describe.concurrent('StrucvarDetailsView', async () => {
  ...
  it('renders the loading state', async () => {
    ...
    // assert:
    expect(wrapper.html()).toContain('<strucvar-gene-list-card-stub ')
  })
  ...
  it('renders SvDetails info-cards', async () => {
    ...
    expect(wrapper.html()).toContain(
      'strucvar-gene-list-card-stub csq="[object Object],[object Object],[object Object],[object Object]" genes-infos="[object Object]" store-state="active"></strucvar-gene-list-card-stub>'
    )
    expect(wrapper.html()).toContain(
      '<strucvar-clinvar-card-stub strucvar="[object Object]"></strucvar-clinvar-card-stub>'
    )
    expect(wrapper.html()).toContain(
      '<strucvar-tools-card-stub strucvar="[object Object]"></strucvar-tools-card-stub>'
    )
    expect(wrapper.html()).toContain(
    ...
    expect(wrapper.html()).toContain(
      '<genome-browser-card-stub genome-build="grch37" locus="17:41175312-41278500"></genome-browser-card-stub>'
    )
  })
})
